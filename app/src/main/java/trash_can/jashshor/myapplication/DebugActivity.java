...
//        setTitle(winTitle);   														//改变标题 暂定
        ...
        ((Button)findViewById(R.id.testc)).setOnClickListener(this);
        ((Button)findViewById(R.id.testj)).setOnClickListener(this);
        ...
        if (devfd >= 0) {
            timer.schedule(task, 0, 100);  //设置 timer ：0延时，500周期——重复task
        } else {
            devfd = -1;
                ...
                case 1:
                    if (HardwareControler.select(devfd, 0, 0) == 1) {       //以下：timer 通过调用 select 接口轮询串口设备是否有数据到来
                        ...
                        if (retSize > 0) {
                            ...
                            {
                                case "red": yanse.setText("颜色传感器：红色");break;
                                case "blue": yanse.setText("颜色传感器：蓝色");break;
                                case "green": yanse.setText("颜色传感器：绿色");break;
                                case "none": yanse.setText("颜色传感器：其他");break;
                            }
                            if(str.contains("ig")) dianji.setText("伺服电机：工作");
                            if(str.contains("ibug")) dianji.setText("伺服电机：未工作");
                            ...
                            if(str.contains("juli")) {                              //距离传入
                                ...
                                Log.e("完全净化，接收的",str);
                                double in = Integer.parseInt(str);                //String字面转Int
                                juli.setText("距离传感器："+String.valueOf(in)+"mm");
                            }else Log.e("输入","没有实质性数据");
                            }
        ...
        switch (v.getId()) {					//识别点击按钮点击按钮ID
            ...
            case R.id.test:
                ...
                if (str2.length() > 0) {
                    ...
                    int ret = HardwareControler.write(devfd, str2.getBytes());       //转化二进制调用write接口发送 //TODO 发送数据方式
                    Log.i("write","str");
                    if (ret > 0) {              //依据发送成功与否分类
                        toEditor.setText("");           //TODO setText("") 清空EditText
                        str2 = "> " + str2;
                        ...
                        fromTextView.append(str2);       //在显示上和数据存储上按'“>>>”+?'格式增加发送值（实际上也没必要区分成两个量）
                    } else {
                        Toast.makeText(this,"Fail to send!",Toast.LENGTH_SHORT).show();
                    }
                }break;
            ...
            case R.id.testc:
                ...
                if (str3.length() > 0) {
                    ...
                    int ret = HardwareControler.write(devfd, str3.getBytes());       //转化二进制调用write接口发送 //TODO 发送数据方式
                    Log.i("write","str");
                    if (ret > 0) {              //依据发送成功与否分类
                        toEditor.setText("");           //TODO setText("") 清空EditText
                        str3 = "> " + str3;
                        ...
                        remoteData.append(str3);
                        fromTextView.append(str3);       //在显示上和数据存储上按'“>>>”+?'格式增加发送值（实际上也没必要区分成两个量）
                    } else {
                        Toast.makeText(this,"Fail to send!",Toast.LENGTH_SHORT).show();
                    }
                }break;
            case R.id.testj:
                ...
                if (str4.length() > 0) {
                    ...
                    int ret = HardwareControler.write(devfd, str4.getBytes());       //转化二进制调用write接口发送 //TODO 发送数据方式
                    Log.i("write","str");
                    if (ret > 0) {              //依据发送成功与否分类
                        toEditor.setText("");           //TODO setText("") 清空EditText
                        str4 = "> " + str4;
                        ...
                        remoteData.append(str4);
                        fromTextView.append(str4);       //在显示上和数据存储上按'“>>>”+?'格式增加发送值（实际上也没必要区分成两个量）
                    } else {
                        Toast.makeText(this,"Fail to send!",Toast.LENGTH_SHORT).show();
                    }
                }break;
